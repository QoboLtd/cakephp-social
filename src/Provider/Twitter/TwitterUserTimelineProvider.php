...
class TwitterUserTimelineProvider extends AbstractTwitterProvider
{
    ...
    public function read(array $options = []): ResponseInterface
    {
        ...
        $json = (string)file_get_contents('/home/asaliev/Work/cakephp-social/twitter_user_timeline_test.json');
        // $payload = $this->callApi($credentials['oauthToken'], $credentials['oauthTokenSecret'], $config);
        $payload = new stdClass;
        $payload->results = json_decode($json);
        // file_put_contents('/home/asaliev/Work/cakephp-social/twitter_user_timeline_test.json', json_encode($payload));
